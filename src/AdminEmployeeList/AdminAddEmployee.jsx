...
import { FiX, FiUpload, FiUser, FiShield, FiCreditCard, FiClock, FiPlus, FiTrash2, FiChevronDown, FiCheck, FiSearch } from 'react-icons/fi';
import {
  addEmployee,
  uploadPhoto,
  addEmployeeProof,
...
import styles from './AdminAddEmployee.module.css';
...
// AddEmployee
// ────────────────────────────────────────────────
const AddEmployee = ({ isOpen, onClose, onSuccess, onError }) => {
  ...
  // ── Step 0: Clinic + Branch selection ──
  const [selectedClinicId,    setSelectedClinicId]    = useState(null);
  const [selectedClinicName,  setSelectedClinicName]  = useState('');
  const [selectedBranchId,    setSelectedBranchId]    = useState(null);
  const [selectedBranchName,  setSelectedBranchName]  = useState('');

  // Clinic searchable dropdown state
  const [clinicSearchText,    setClinicSearchText]    = useState('');
  const [clinicList,          setClinicList]          = useState([]);
  const [clinicDropdownOpen,  setClinicDropdownOpen]  = useState(false);
  const [clinicDropdownStyle, setClinicDropdownStyle] = useState({});
  const clinicSearchRef = useRef(null);

  // Branch dropdown
  const [branchList, setBranchList] = useState([]);

  // Departments scoped to selected clinic+branch
  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      if (clinicSearchRef.current && !clinicSearchRef.current.contains(e.target)) {
        ...
        setClinicSearchText(selectedClinicName);
      }
    };
    document.addEventListener('mousedown', handleClickOutside);
    return () => document.removeEventListener('mousedown', handleClickOutside);
  ...
  // ── Open clinic dropdown with fixed positioning ──
  const openClinicDropdown = useCallback(() => {
    if (clinicSearchRef.current) {
      const rect = clinicSearchRef.current.getBoundingClientRect();
      setClinicDropdownStyle({ top: rect.bottom + 4, left: rect.left, width: rect.width });
    }
    setClinicDropdownOpen(true);
  }, []);

  // ── Fetch all clinics on mount (preload) ──
  ...
  useEffect(() => {
    if (isOpen) fetchClinicsByName('');
  }, [isOpen]);

  // ── Fetch shifts once clinic is confirmed ──
  const fetchShifts = async (clinicId) => {
    setShiftsLoading(true);
    try {
      const list = await getShiftList(clinicId, { Status: 1 });
      setShifts(list);
    } catch { setShifts([]); }
    finally { setShiftsLoading(false); }
  };

  // ── Fetch branches when clinic is selected ──
  useEffect(() => {
    if (!selectedClinicId) { setBranchList([]); setSelectedBranchId(null); setSelectedBranchName(''); return; }
    const fetchBranches = async () => {
      try {
        const data = await getBranchList(selectedClinicId, { BranchType: 0, PageSize: 200 });
        setBranchList(data);
  ...
  // ── Fetch departments when clinic+branch is set ──
  useEffect(() => {
    if (!selectedClinicId) { setDepartments([]); return; }
    const fetchDepts = async () => {
      try {
        const data = await getDepartmentList(selectedClinicId, selectedBranchId || 0, {});
        setDepartments(data);
  ...
  const resetForm = () => {
    ...
    setSelectedClinicId(null); setSelectedClinicName('');
    setSelectedBranchId(null); setSelectedBranchName('');
    setClinicSearchText(''); setClinicList([]); setClinicDropdownOpen(false);
    setBranchList([]); setDepartments([]);
  ...
  // ── Step 0 valid: clinic required, branch optional ──
  const isStep0Valid = selectedClinicId !== null && selectedBranchId !== null;
  ...
  const handlePhotoUploadSubmit = async () => {
    ...
    try {
      ...
      const res = await uploadPhoto(selectedClinicId, photo, fileAccessToken);
  ...
  const handleClinicSelect = (clinic) => {
    ...
    setClinicDropdownOpen(false);
    // Reset branch when clinic changes
  ...
  const handleBranchChange = (e) => {
    ...
    if (val === '') {
      ...
      const branch = branchList.find(b => String(b.id) === val);
  ...
  // ── Step 0 Submit: confirm clinic+branch, fetch shifts, move to step 1 ──
  const handleStep0Submit = (e) => {
    e.preventDefault();
    if (!isStep0Valid) {
      showPopup('Please select a Clinic and Branch before continuing.', 'warning');
      return;
    }
    fetchShifts(selectedClinicId);
    setCurrentStep(1);
  };
  ...
  const handleStep4Submit = async (e) => {
    ...
    try {
      for (const shiftId of selectedShifts) {
        await addEmployeeShift({ ClinicID: selectedClinicId, EmployeeID: createdEmployeeId, ShiftID: Number(shiftId) });
      }
      for (const dayId of selectedWorkDays) {
        await addWorkDays({ ClinicID: selectedClinicId, EmployeeID: createdEmployeeId, WorkDay: dayId });
      }
  ...
  const handlePrevious = () => {
    if (currentStep > 1) {
      ...
      // Go back to step 0 (clinic/branch selection)
      setCurrentStep(0);
      setError(null); setStepSuccess('');
      setSubmitAttempted(false);
    }
  ...
  return (
    <div className={styles.overlay}>
      ...
      <div className={styles.modal}>
          ...
          {selectedClinicName && (
            <div className={styles.clinicNameone}>
              ...
              <div className={styles.clinicNameStack}>
                <span className={styles.clinicNamePrimary}>{selectedClinicName}</span>
                {selectedBranchName && (
                  <span className={styles.clinicNameSecondary}>{selectedBranchName}</span>
                )}
              </div>
            </div>
          )}
        ...
        {/* ────────── STEP 0: Clinic + Branch ────────── */}
        {currentStep === 0 && (
          <form className={styles.form} onSubmit={handleStep0Submit}>
            <div className={styles.body}>
              <div className={styles.formSection}>
                <div className={styles.formSectionHeader}>
                  <h3><FaClinicMedical size={15} /> Select Clinic &amp; Branch</h3>
                </div>
                <p className={styles.formHint} style={{ marginBottom: 4 }}>
                  Select the clinic (and optionally a branch) this employee belongs to. This will be used for all steps.
                </p>

                <div className={styles.formGrid} style={{ gridTemplateColumns: '1fr 1fr' }}>

                  {/* Clinic searchable input */}
                  <div className={styles.formGroup}>
                    ...
                    <div className={styles.clinicSearchGroup} ref={clinicSearchRef}>
                      ...
                      />
                      {clinicSearchText !== '' ? (
                        <button type="button" className={styles.clinicClearBtn} onClick={handleClinicClear} tabIndex={-1}>
                          <FiX size={13} />
                        </button>
                      ) : (
                        <button type="button" className={styles.clinicClearBtn} onClick={handleClinicSearchIconClick} tabIndex={-1} title="Search clinics">
                          <FiSearch size={13} />
                        </button>
                      )}
                      {clinicDropdownOpen && (
                        <div className={styles.clinicDropdown} style={clinicDropdownStyle}>
                          {clinicList.length === 0 ? (
                            ...
                            clinicList.map(clinic => (
                              <div
                                key={clinic.id}
                                className={`${styles.clinicDropdownItem} ${selectedClinicId === clinic.id ? styles.clinicDropdownItemSelected : ''}`}
            ...
            <div className={styles.footer}>
              ...
              <button
                ...
                title={!selectedClinicId ? 'Please select a clinic' : !selectedBranchId ? 'Please select a branch' : ''}
              >
                Continue
              </button>
            </div>
        ...
        {currentStep === 1 && (
          <form className={styles.form} onSubmit={handleStep1Submit}>
            ...
            {renderFooter('Save & Next', false, false, isStep1Valid)}
          </form>
        )}
