...
const UpdateClinic = ({ clinic, onClose, onSuccess, onError }) => {
  ...
  // ── Change logo state (only active when user clicks Change Logo) ──
  const [isChangingLogo,     setIsChangingLogo]     = useState(false);

  // ── New upload state (used when no existing logo OR when changing logo) ──
  ...
  // ── New logo file selection (used when no existing logo OR when changing logo) ──
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Upload new logo when: no existing logo with file selected, OR changing logo with file selected
      if ((!existingLogoFileId && newLogoFile) || (isChangingLogo && newLogoFile)) {
        ...
        try {
          ...
          setExistingLogoUrl(null); // clear cached URL so next view re-fetches
          setNewLogoFile(null);
          setNewLogoPreviewUrl(null);
          setIsChangingLogo(false);
        } catch (uploadErr) {
            ...
            {existingLogoFileId > 0 && !isChangingLogo ? (
              ...
              <div className={styles.logoExistingRow}>
                ...
                <div className={styles.logoActionBtns}>
                  <button
                    type="button"
                    className={styles.logoViewBtn}
                    onClick={handleViewLogo}
                    disabled={logoFetching}
                  >
                    <FiEye size={14} />
                    {logoFetching ? 'Loading...' : 'View Logo'}
                  </button>
                  <button
                    type="button"
              ...
              /* ── Has logo & changing → show compact upload + cancel ── */
              <>
                <input
                  ref={changeLogoInputRef}
                  type="file"
                  accept=".jpg,.jpeg,.png"
                  className={styles.logoHiddenInput}
                ...
                <div className={styles.photoUploadRow}>
                  ...
                  <div
                    ...
                    onClick={() => changeLogoInputRef.current?.click()}
                  ...
                  <div className={styles.photoUploadRight}>
                    ...
                    >
                      <FiUpload size={13} />
                      {newLogoFile ? 'Change Photo' : 'Select Photo'}
                    </button>
                    <span className={styles.photoFormatHint}>JPG, JPEG or PNG · Max 4MB</span>
                    {newLogoFile && (
                      <span className={styles.photoSelectedName}>{newLogoFile.name}</span>
                    )}
                    <button
                      type="button"
                      className={styles.logoCancelChangeBtn}
